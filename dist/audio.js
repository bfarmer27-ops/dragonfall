...
export function createAudio(){
 ...
 const api={
  ...
  // Passing a ring: a three-partial bell with a little reverb tail.
  gate(){if(!ctx||!enabled)return;for(const [ratio,level,dur] of [[1,.28,1.1],[2.71,.12,.7],[5.4,.05,.45]])tone({freq:880*ratio,duration:dur,level,type:'sine'});burst({kind:'white',duration:.08,from:6000,to:2500,level:.06});},
