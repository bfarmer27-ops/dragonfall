...
- Dragon: **Demon** (default) is a downloaded model, "Demon Dragon Full Texture" by endlessvoidmc, CC-BY 4.0, thinned to 40,067 triangles with only its flying clip (`dist/assets/models/demon-dragon.glb`, credits in `CREDITS.txt`), driven by `dragon-gltf.js`; **Classic** is the code-built dragon (`dragon.js`). Settings > Dragon; saved in localStorage `dragonfall-dragon`; `?dragon=classic|demon` for a one-load test.
- Features (2026-09-10): synthesized sounds (`audio.js`), voice fire word (`speech.js`, default "dracarys", Settings > Fire breathing) plus the on-screen FIRE button and the F key, fireballs that shatter boulders and count KILLS (`fireball.js`), friend rooms over WebRTC (`net.js`, PeerJS public relay: Settings > Fly with friends, 4-letter code, `?room=CODE` link, remote riders, scoreboard, respawn), Settings > Flight speed slider, landscape vertical gain, nose-dive camera feedback, reins that pull with the thumbs.
...
- Tests: `node --test tests/*.test.mjs` (15). Two-browser multiplayer check: `node D:\AgentWork\dragonfall
et-test.mjs` (outside the repo). Dev pages under `dist/dev/` load each module alone. Debug overlay: add `?stats=1` to the URL.
